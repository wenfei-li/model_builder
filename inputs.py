...
from model_builder.models.structurebasedmodel import StructureBasedModel as SBM
from model_builder.models.awsemmodel import AwsemModel as AwsemModel
...
def _load_sbm(modelopts):
    ...
    # add a flavored interaction based on atom-type LJ interactions:
    if modelopts["flavored_interactions_file"] is not None:
        flavor_file = open(modelopts["flavored_interactions_file"], "r")
        type_dict = {}
        for line in flavor_file:
            if line[0] == "#":
                pass # comment lines ignored
            else:
                ...
                type_dict[name] = values

        model.use_flavored_nonnative_interactions(type_dict)
...
def _empty_model_opts():
    ...
    opts = ["pairs_file","pairwise_params_file",
            ...
            "using_sbm_gmx","umbrella", "topology", "reference", "parameters_to_fit_file", "flavored_interactions_file"]
...
def parse_pairwise_params(pairwise_file):
    ...
    from model_builder.models.potentials.pairwise import PAIR_POTENTIALS
